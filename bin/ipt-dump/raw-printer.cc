...
std::unique_ptr<RawPrinter> RawPrinter::Create(
    DecoderState* state,
    const PrinterConfig& config)
{
  ...
  auto printer = std::unique_ptr<RawPrinter>(
      new RawPrinter(out_file, state, config));
  return printer;
}
...
RawPrinter::Space RawPrinter::GetSpace(uint64_t cr3, const Symtab* symtab)
{
  if (symtab) {
    if (symtab->is_kernel)
...
{
  Printf("%" PRIu64 ": %" PRIx64 ": %s",
	 ps->current_ts, ps->current_pc, InsnClass(insn->iclass));
...
int RawPrinter::ProcessNextInsn(struct pt_insn_decoder* pt_decoder,
  ...
  insn.ip = 0;
  int err = pt_insn_next(pt_decoder, &insn, sizeof(struct pt_insn));
  uint64_t cr3;
  ...
  const Symtab* symtab = state_->FindSymtab(ps->current_cr3, ps->current_pc);
  const Sym* sym = symtab ? symtab->FindSym(ps->current_pc) : nullptr;
  ...
  if (sym != ps->current_function) {
    if (sym) {
      Printf("Current function is now %s:%s\n",
	     symtab->file_name ? symtab->file_name : "unknown",
...
uint64_t RawPrinter::PrintOneFile(const PtFile& pt_file)
{
  ...
  for (;;) {
	...
	ftl::StringPrintf("0x%" PRIx64 ": sync forward: %s\n",
                          ps.current_pos, pt_errstr(pt_errcode(err)));
      if (err == -pte_eos) {
