...
bool RspServer::Run() {
  ...
  io_loop_ = std::make_unique<RspIOLoop>(client_sock_.get(), client_sock_.get(), this);
  io_loop_->Run();
...
void RspServer::QueueNotification(
    const ftl::StringView& name,
    const ftl::StringView& event,
    const ftl::TimeDelta& timeout) {
...
void RspServer::QueueStopNotification(
    const ftl::StringView& event,
    const ftl::TimeDelta& timeout) {
...
void RspServer::OnProcessOrThreadExited(Process* process,
                                        Thread* thread,
...
void RspServer::OnArchitecturalException(Process* process,
                                         Thread* thread,
                                         const mx_excp_type_t type,
                                         const mx_exception_context_t& context) {
  FTL_DCHECK(process);
  FTL_DCHECK(thread);
  FTL_VLOG(1) << "Architectural Exception: " << util::ExceptionToString(type, context);
